...
A unified GTK4 shell for the [Niri](https://github.com/YaLTeR/niri) Wayland compositor. Combines a panel, dock, launcher, notification daemon, quick settings, polkit agent, and power management into a single process that connects to Niri via its IPC socket for live compositor state.
...
- **Dock** — vertical bar showing active Niri windows and pinned apps; highlights the focused window; persists pins to `~/.config/niri-shell/pins.json`; shows unread notification badges
...
src/
  ...
  dock_ui.rs                 — GTK4 dock window (layer shell, vertical bar)
...
Backends (`panel_backend`, `dock_backend`, `launcher_backend`, `quick_settings_backend`, `power_backend`, `notification_daemon`, `polkit_agent`) contain pure Rust logic with no GTK4 dependency and are fully unit-testable.
...
- **File Manager** — GTK4, no Adwaita; two-pane layout with bookmarks sidebar, icon/list/column view, Dolphin-style breadcrumb, and XDG trash support
- **Niri Settings GUI** — visual editor for `~/.config/niri/config.kdl`; keybindings recorder, window rules, output layout, and theme token live preview
